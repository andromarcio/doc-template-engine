...
7. **Numere os critérios de aceite em sequência simples dentro da sua história** (1, 2, 3…), reiniciando a contagem a cada história, com a única finalidade de separar um critério do outro e permitir citá-los na conversa. Nada além disso: **sem padrão de identificação** (nada de CRIT.01.01) e **sem títulos ou nomes** como "Nome da ata" ou "Salvar parâmetros". As **histórias** seguem a mesma lógica — número sequencial simples (História 1, História 2, História 3…) apenas para separar uma da outra no documento gerado.
...
Ligado: cada elemento rastreável (bloco, campo, ação, mensagem, modal) exibe uma etiqueta com a história de origem e o número do critério de aceite que atende (ex.: História 2 · critério 3). Ao apontar para a etiqueta, exibe-se o texto do critério — como os critérios não têm título (regras 2 e 7), o balão traz o texto na íntegra. Uma matriz de rastreabilidade (história → critério → onde aparece na tela) é exibida ao final da página.
...
Fonte de verdade: o texto dos critérios exibido no protótipo é uma cópia de conveniência; o documento de requisitos (histórias e critérios de aceite) prevalece. Ao atualizar um critério, atualizar as duas peças na mesma iteração, mantendo a mesma numeração de histórias e de critérios nos dois artefatos.
